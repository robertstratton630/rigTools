...
def getSkinClusterFn(skinCluster):
    ...
    skinClusterSel = OpenMaya.MGlobal.getSelectionListByName(skinCluster)
    skinClusterObj = skinClusterSel.getDependNode(0)
...
def getSkinClusterInfluences(skincluster):
    skinFn = getSkinClusterFn(skincluster)
    influencePaths = skinFn.influenceObjects()
    infNameArray = [influencePaths[i].partialPathName()
                    for i in range(len(influencePaths))]
    return infNameArray
...
def getInfluenceAtIndex(skinCluster, influenceIndex):
    ...
    infConn = cmds.listConnections(
        skinCluster+'.matrix['+str(influenceIndex)+']', s=True, d=False)

    # Check Connection
    if not infConn:
        raise Exception('No influence at specified index!')
...
def getInfluencePhysicalIndex(skinCluster, influence):
    ...
    infNameArray = [infPathArray[i].partialPathName()
                    for i in range(infPathArray.length())]

    # Check influence
    if not influence in infNameArray:
        raise Exception(
            'Unable to determine influence index for "'+influence+'"!')
...
def getInfluenceWeights(skinCluster, influence, componentList=[]):
    ...
    # Get geometry
    affectedGeo = glTools.utils.deformer.getAffectedGeometry(skinCluster).keys()[
        0]
    ...
    skinFn.getWeights(
        componentSel[0], componentSel[1], influenceIndex, weightList)
...
def getInfluenceWeightsAll(skinCluster, componentList=[]):
    ...
    skinFn.getWeights(
        componentSel[0], componentSel[1], weightList, infCountPtr)
...
def getInfluenceWeightsSlow(skinCluster, influence, componentList=[]):
    ...
    # Get weight values
    weightList = [cmds.skinPercent(skinCluster, affectedGeo+'.vtx['+str(i)+']',
                                 transform=influence, q=True) for i in componentIndexList]
...
def setInfluenceWeights(skinCluster, influence, weightList, normalize=True, componentList=[]):
    ...
    skinFn.setWeights(componentSel[0], componentSel[1],
                      infIndexArray, wtArray, normalize, oldWtArray)
...
def setInfluenceWeightsSlow(skinCluster, influence, weightList, normalize=True, componentList=[]):
    ...
    for i in range(len(componentIndexList)):
        ...
        cmds.skinPercent(skinCluster, comp, tv=(
            influence, weightList[i]), normalize=normalize)
...
def setInfluenceWeightsAll(skinCluster, weightList, normalize=True, componentList=[]):
    ...
    infIndexArray = OpenMaya.MIntArray()
    [infIndexArray.append(getInfluencePhysicalIndex(
        skinCluster, inf)) for inf in influenceList]
    infDict = {}
    ...
    skinFn.setWeights(componentSel[0], componentSel[1],
                      infIndexArray, wtArray, False, oldWtArray)
...
def lockInfluenceWeights(influence, lock=True, lockAttr=False):
    ...
    if not cmds.attributeQuery('liw', n=influence, ex=True):
        raise Exception('Influence ("'+influence +
                        '") does not contain attribute "lockInfluenceWeights" ("liw")!')
    ...
    try:
        ...
        if lockAttr:
            cmds.setAttr(influence+'.liw', l=True)
    except:
        pass
...
def lockSkinClusterWeights(skinCluster, lock=True, lockAttr=False):
    ...
    # Check SkinCluster
    if not isSkinCluster(skinCluster):
        raise Exception('Object "'+skinCluster +
                        '" is not a valid skinCluster node!')
...
def lockSkinClusterWeightsFromGeo(geo, lock=True, lockAttr=False):
    ...
    # Lock Weights
    influenceList = lockSkinClusterWeights(
        skinCluster, lock=lock, lockAttr=lockAttr)
...
def getAffectedPoints(skinCluster, influence):
    ...
    weightList = OpenMaya.MDoubleArray()
    skinClusterFn.getPointsAffectedByInfluence(
        influencePath, pointSel, weightList)
...
def rename(geometry, suffix='skinCluster'):
    ...
    # Get name prefix
    # glTools.utils.stringUtils.stripSuffix(geometry)
    prefix = geometry.split(':')[-1]
...
def reset(geometry):
    ...
    skinCluster = cmds.skinCluster(
        geometry, influenceList, dr=4, mi=maxInfluences, omi=useMaxInfluences, tsb=True)
...
def mirrorSkinWIP(	srcGeo,
    ...
    if not srcSkin:
        raise Exception(
            'No skinCluster found for source geometry "'+srcGeo+'"!')
    ...
    if not dstSkin:
        dstSkin = dstGeo.split(':')[-1]+'_skinCluster'
...
def mirrorSkin(skinCluster, search='lf_', replace='rt_', destGeo=''):
    ...
    sourceGeo = glTools.utils.deformer.getAffectedGeometry(skinCluster).keys()[
        0]
    ...
    if not cmds.objExists(mSkinCluster):
        # Create skinCluster
        mSkinCluster = cmds.skinCluster(
            mInfluenceList, destGeo, tsb=True, n=mSkinCluster)[0]
    else:
        # Add influence
    ...
    for i in range(len(influenceList)):
        setInfluenceWeights(
            mSkinCluster, mInfluenceList[i], mirroWeightList[influenceList[i]])


def createMirrorInfluenceList(influenceList, searchJointList=None, flipAxis=[-1, 1, 1]):

    # Check Joint List
    if not searchJointList:
        searchJointList = cmds.ls(type='joint')
    ...
    for i in range(len(influenceList)):
        ...
        for jnt in searchJointList:
            ...
            dist = glTools.utils.mathUtils.distanceBetween(
                point1=mirrorPos, point2=testPos)
...
def mirrorSkinToNewGeom(	srcSkin,
    ...
    if not cmds.objExists(dstSkin):
        # Create SkinCluster
        dstSkin = cmds.skinCluster(
            mInfluenceList, dstGeo, tsb=True, n=dstSkin)[0]
    else:
        # Add Influence
        dstInfluenceList = cmds.skinCluster(dstSkin, q=True, inf=True)
        for mInf in mInfluenceList:
...
def bindPreMatrix(skinCluster, influence, influenceBase=None, parent=True):
    ...
    if influenceBase:
        if not cmds.objExists(influenceBase):
            raise Exception('Influence base "' +
                            influenceBase+'" does not exist!')
    if not influenceBase:
        influenceBase = influence
    ...
    if influence == influenceBase:
        ...
        cmds.connectAttr(
            influence+'.parentInverseMatrix[0]', skinCluster+'.bindPreMatrix['+str(infIndex)+']', f=True)
        return influence
    ...
    if influenceBase:
        ...
        bpm = cmds.createNode('transform', n=influence.replace(
            influence.split('_')[-1], '')+'bindPreMatrix')
    ...
    try:
        cmds.connectAttr(
            bpm+'.worldInverseMatrix[0]', skinCluster+'.bindPreMatrix['+str(infIndex)+']', f=1)
    except:
        pass
...
def makeRelative(skinCluster, relativeTo):
    ...
    if not cmds.objExists(relativeTo):
        ...
        if cmds.objectType(relativeTo) != 'transform':
            raise Exception('Object "'+relativeTo +
    ...
    for inf in influenceList:
        ...
        infInd = -1
        plugConnection = cmds.listConnections(
            inf+'.worldMatrix[0]', s=0, d=1, p=1, type="skinCluster")
        ...
        # Connect bindPreMatrix to skinCluster
        bpmConn = cmds.listConnections(
            skinCluster+'.bindPreMatrix['+str(infInd)+']', s=1, d=0)
        if type(bpmConn) == list:
            if bpm != bpmConn[0]:
                cmds.connectAttr(
                    bpm+'.worldInverseMatrix[0]', skinCluster+'.bindPreMatrix['+str(infInd)+']', f=1)
        else:
            cmds.connectAttr(
                bpm+'.worldInverseMatrix[0]', skinCluster+'.bindPreMatrix['+str(infInd)+']', f=1)
...
def skinAs(src, dst, smooth=False):
    ...
    if not dstSkin:
        ...
        dstSkin = cmds.skinCluster(
            srcInfList, dst, toSelectedBones=True, rui=False, n=dstPrefix+'_skinCluster')[0]
...
def skinObjectList(objList, jntList):
    ...
    for obj in objList:
        ...
        # Create SkinCluster
        skinCluster = cmds.skinCluster(
            jntList, obj, toSelectedBones=True, rui=False, n=objName+'_skinCluster')[0]
...
def clean(skinCluster, tolerance=0.005):
    ...
    if not geoShape:
        raise Exception(
            'Unable to determine deformed geometry from skinCluster "'+skinCluster+'"!')
    geo = cmds.listRelatives(geoShape[0], p=True, pa=True)
    if not geo:
        raise Exception(
            'Unable to determine geometry from deformed shape "'+geoShape[0]+'"!')
    # Select Geometry
    cmds.select(geo[0])
...
def removeMultipleInfluenceBases(base, duplicates):
    ...
    if not baseShape:
        raise Exception(
            'Unable to determine base influence shape for "'+base+'"!')
    ...
    for dup in duplicates:
        ...
        if not dupShape:
            print('Unable to determine base influence shape for "' +
                  base+'"! Skipping...')
            continue

        # Get SkinCluster Connection
        dupConn = cmds.listConnections(
            dup+'.outMesh', s=0, d=1, p=True, type='skinCluster')
        if not dupConn:
            print('Base influence "'+dup +
                  '" deleted! No outgoing skinCluster connections...')
            cmds.delete(dup)
            continue
        ...
        try:
            ...
            print('Base influence connection to skinCluster"' +
                  dupConnSkin+'" overridden ('+dup+' >>> '+base+' )')
        except:
            print('Unable to override base influence connection to skinCluster"' +
                  dupConnSkin+'" ('+dup+' >>> '+base+' )! Skipping...')
            continue
...
def replaceGeomInfluenceBase(skinCluster, influence, influenceBase, deleteOldBase=False):
    ...
    if not infList.count(influence):
        raise Exception(
            'Object "'+influence+'" is not a valid influence of skinCluster "'+skinCluster+'"!')
    ...
    oldBase = cmds.listConnections(
        skinCluster+'.basePoints['+str(infIndex)+']', s=True, d=False)

    # Connect to SkinCluster
    cmds.connectAttr(influenceBase+'.outMesh', skinCluster +
                   '.basePoints['+str(infIndex)+']', f=True)
